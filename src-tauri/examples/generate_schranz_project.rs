//! Generate a Schranz project - flat structure, aggressive sounds, 155 BPM
...
fn main() {
    let output_path = Path::new("/Users/wizard/Desktop/Schranz_Project.als");

    match generate_schranz_project(output_path) {
        Ok(()) => {
            ...
            println!("├── BASS ({} clips) - rumble/sub", CLIPS_PER_TRACK);
            println!("├── MELODICS (Group)");
            println!("│   ├── SYNTH 1 ({} clips) - stabs", CLIPS_PER_TRACK);
            println!("│   ├── SYNTH 2 ({} clips) - leads", CLIPS_PER_TRACK);
            println!("│   └── PAD ({} clips) - minimal", CLIPS_PER_TRACK);
            println!("├── FX (Group)");
...
fn generate_schranz_project(output_path: &Path) -> Result<(), String> {
