...
// ── Native drag-out: drop samples onto DAW tracks / Finder (tauri-plugin-drag) ──
const _AUDIO_DRAG_ICON_PNG = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==';
const _AUDIO_DRAG_THRESHOLD_SQ = 8 * 8;
let _audioDragPointer = null;

function audioDragPathsForRow(rowPath) {
  if (typeof batchSelected !== 'undefined' && batchSelected.size > 0 && batchSelected.has(rowPath)) {
    return [...batchSelected];
  }
  return [rowPath];
}

async function startNativeAudioDrag(filePaths) {
...
function initAudioNativeFileDrag() {
  if (typeof document === 'undefined' || initAudioNativeFileDrag._done) return;
  initAudioNativeFileDrag._done = true;

  document.addEventListener('pointerdown', (e) => {
    if (e.button !== 0) return;
    const samplesTab = document.getElementById('tabSamples');
    if (!samplesTab || !samplesTab.classList.contains('active')) return;
    const row = e.target.closest('#audioTableBody tr[data-audio-path]');
    if (!row || row.id === 'audioLoadMore') return;
  ...
  document.addEventListener('pointermove', (e) => {
    if (!_audioDragPointer || e.pointerId !== _audioDragPointer.pointerId) return;
    const d = _audioDragPointer;
    const dx = e.clientX - d.x;
    const dy = e.clientY - d.y;
    if (dx * dx + dy * dy < _AUDIO_DRAG_THRESHOLD_SQ) return;
    if (d.didDrag) return;
    d.didDrag = true;
    e.preventDefault();
    void startNativeAudioDrag(audioDragPathsForRow(d.path));
  }, true);

  document.addEventListener('pointerup', (e) => {
    if (!_audioDragPointer || e.pointerId !== _audioDragPointer.pointerId) return;
    const d = _audioDragPointer;
    _audioDragPointer = null;
    if (d.didDrag && typeof window !== 'undefined') {
      window.__suppressAudioMetadataToggle = true;
      setTimeout(() => {
        if (typeof window !== 'undefined') window.__suppressAudioMetadataToggle = false;
      }, 500);
    }
  }, true);

  document.addEventListener('pointercancel', (e) => {
    if (_audioDragPointer && e.pointerId === _audioDragPointer.pointerId) {
      _audioDragPointer = null;
    }
  }, true);
}
...
if (typeof document !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => initAudioNativeFileDrag());
  } else {
    initAudioNativeFileDrag();
  }
}
