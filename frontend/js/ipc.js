...
window.vstUpdater = {
    ...
    getAlsBlacklistEntries: () => invoke('get_als_blacklist_entries'),
    addToAlsBlacklist: (path) => invoke('add_to_als_blacklist', {path}),
    removeFromAlsBlacklist: (entry) => invoke('remove_from_als_blacklist', {entry}),
    // Directory whitelist
    getAlsWhitelistEntries: () => invoke('get_als_whitelist_entries'),
    getAlsWhitelistCount: () => invoke('get_als_whitelist_count'),
    addToAlsWhitelist: (path) => invoke('add_to_als_whitelist', {path}),
    ...
    crateSimilarCandidates: (sampleId, candidateLimit) => invoke('crate_similar_candidates', {sampleId, candidateLimit: candidateLimit || 500}),
    findSimilarSamples: (filePath, candidatePaths, maxResults) => invoke('find_similar_samples', {filePath, candidatePaths, maxResults}),
    // Waveform prefetch — bulk fill of the waveform_cache so rows never wait on decode.
    waveformPrefetchStart: () => invoke('waveform_prefetch_start'),
    waveformPrefetchStop: () => invoke('waveform_prefetch_stop'),
    waveformPrefetchStats: () => invoke('waveform_prefetch_stats'),
    onWaveformPrefetchProgress: (callback) => listen('waveform-prefetch-progress', (event) => callback(event.payload)),
    ...
    favoritesList: () => invoke('favorites_list'),
    favoritesAdd: (favType, path, name, format, daw, addedAt) => invoke('favorites_add', {favType, path, name, format, daw, addedAt}),
    favoritesRemove: (path) => invoke('favorites_remove', {path}),
    ...
    notesGetAll: () => invoke('notes_get_all'),
    // Tags (SQLite-backed)
    tagsStandaloneList: () => invoke('tags_standalone_list'),
    tagsStandaloneSet: (tags) => invoke('tags_standalone_set', {tags}),
    tagsStandaloneAdd: (tag) => invoke('tags_standalone_add', {tag}),
    tagsStandaloneRemove: (tag) => invoke('tags_standalone_remove', {tag}),
    tagsAll: () => invoke('tags_all'),
    tagsCounts: () => invoke('tags_counts'),
    tagsItemsWith: (tag) => invoke('tags_items_with', {tag}),
    tagRename: (oldTag, newTag) => invoke('tag_rename', {oldTag, newTag}),
    tagDelete: (tag) => invoke('tag_delete', {tag}),
    tagAddToItem: (path, tag) => invoke('tag_add_to_item', {path, tag}),
    tagRemoveFromItem: (path, tag) => invoke('tag_remove_from_item', {path, tag}),
    tagHas: (path, tag) => invoke('tag_has', {path, tag}),
