...
async function scanMidi(resume = false, overrideRoots = null) {
    ...
    try {
        ...
        if (!result.streamed) {
            try {
                await window.vstUpdater.saveMidiScan(allMidiFiles, result.roots);
            } catch (e) {
                if (typeof showToast === 'function' && typeof toastFmt === 'function') showToast(toastFmt('toast.failed_save_midi_history', {err: e.message || e}), 4000, 'error');
            }
        }
        if (_midiScanProgressCleanup) {
            _midiScanProgressCleanup();
            _midiScanProgressCleanup = null;
        }
        _midiScanDbView = false;
        _midiTableInit = false;
        _midiRenderCount = 0;
    ...
    filteredMidi.sort((a, b) => {
        ...
        switch (midiSortKey) {
            ...
            case 'notes':
                ...
                break;
            case 'ch':
                va = ai.channelsUsed || 0;
                vb = bi.channelsUsed || 0;
                break;
            case 'duration':
                va = ai.duration || 0;
                vb = bi.duration || 0;
            ...
            case 'size':
                ...
                break;
            case 'path':
                va = a.directory.toLowerCase();
                vb = b.directory.toLowerCase();
                break;
            default:
                va = a.name.toLowerCase();
                vb = b.name.toLowerCase();
...
function restoreMidiSortState() {
    if (typeof restoreSortState === 'function') {
        ...
        if (saved) {
            midiSortKey = saved.key;
            midiSortAsc = saved.asc;
        }
    }
}
