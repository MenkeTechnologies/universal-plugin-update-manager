...
function reorderNewTableRows(tableId) {
    ...
    for (const row of tbody.rows) {
        ...
        for (const idx of indexMap) {
            if (cells[idx]) frag.appendChild(cells[idx]);
        }
    ...
    initDragReorder(histList, '.np-history-item', null, {
        ...
        onReorder: () => {
            ...
            for (const p of pathOrder) {
                const f = recentlyPlayed.find(r => r.path === p);
...
(function () {
    ...
    document.addEventListener('mouseup', () => {
        ...
        if (c.isDragging) {
            ...
            if (c.placeholder?.parentNode) {
                c.placeholder.parentNode.insertBefore(c.th, c.placeholder);
            ...
            if (typeof prefs !== 'undefined') prefs.setItem(c.prefsKey, {
                v: 2,
    ...
    window.initTableColumnReorder = function (tableId, prefsKey) {
        ...
        if (saved && Array.isArray(saved)) {
            ...
            ths.forEach(th => {
                if (!saved.includes(getColKey(th))) {
                    newOrder.push(ths.indexOf(th));
            ...
            if (tbody && newOrder.length > 0) {
                for (const row of tbody.rows) {
                    ...
                    for (const cell of cells) {
                        if (!frag.contains(cell)) frag.appendChild(cell);
                    }
                    row.appendChild(frag);
                }
            }
