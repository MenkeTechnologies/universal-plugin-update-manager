...
| **Toast Notifications** | Slide-in notifications; user-facing strings live in SQLite (`toast_i18n`: `key`, `locale`, `value`). English defaults are seeded from `src-tauri/toast_i18n_en.json` (regenerate via `scripts/gen_toast_i18n_en.py`). The UI calls `get_toast_strings` on load and resolves messages with `toastFmt('toast.…', { placeholder })`. Add rows with the same keys and another `locale` for translations |
...
| **SQLite Backend** | All data stored in a single `audio_haxor.db` SQLite database (WAL mode, 64MB cache). 14 tables: audio samples, plugins, DAW projects, presets, 4 scan history tables, KVR cache, waveform/spectrogram/xref/fingerprint caches, toast i18n. Designed for 6M+ samples without OOM. Paginated queries with server-side sort/filter/search. Per-cache clear buttons in Settings (BPM, Key, LUFS, Waveform, Spectrogram, Xref, Fingerprint, KVR) |
