...
GitHub Actions (`.github/workflows/ci.yml`) runs `pnpm run test:js`, `cargo test --lib` and `cargo test --test behavioral_ultra` in `src-tauri/`, then `pnpm run tauri:build:ci` (`tauri build --ci --no-sign`) for unsigned release bundles on each OS.
