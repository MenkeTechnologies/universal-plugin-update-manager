...
| **Toast & UI i18n** | Slide-in toasts plus HTML placeholders/titles/options (`data-i18n*`), confirm dialogs, help overlay, and native menu/tray labels load from SQLite `app_i18n` (`key`, `locale`, `value`). English defaults are seeded from `i18n/app_i18n_en.json` (generate via `scripts/gen_app_i18n_en.py`; toast keys are edited in `scripts/gen_toast_i18n_en.py`). German (`de`), Spanish (`es`), Swedish (`sv`), and French (`fr`) defaults are seeded from `i18n/app_i18n_de.json`, `i18n/app_i18n_es.json`, `i18n/app_i18n_sv.json`, and `i18n/app_i18n_fr.json` (regenerate with `scripts/gen_app_i18n_de.py` / `gen_app_i18n_es.py` / `gen_app_i18n_sv.py` / `gen_app_i18n_fr.py` using a venv + `deep-translator`). Settings → Appearance → **Interface language** sets `uiLocale` (`en`, `de`, `es`, `sv`, or `fr`). The UI calls `get_app_strings` on load; `toastFmt` / `appFmt` resolve `toast.*`, `menu.*`, `ui.*`, etc. `applyUiI18n()` applies DOM strings after load. Missing keys fall back to English. Restart the app to apply the native menu bar language after changing locale |
