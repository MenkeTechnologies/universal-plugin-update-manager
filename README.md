...
| **Toast & UI i18n** | Slide-in toasts plus **visible** UI text: `index.html` uses `data-i18n` (labels), `data-i18n-placeholder`, and `data-i18n-title`; `scripts/gen_app_i18n_en.py` extracts strings, merges prior `ui.*` from `i18n/app_i18n_en.json`, injects attributes, and emits the English catalog. Dynamic strings use `appFmt` with keys from `UI_JS_EN` in that script (e.g. plugin scan states, settings Light/Dark/On/Off). Confirm dialogs, help overlay, and native menu/tray labels load from SQLite `app_i18n`. German (`de`), Spanish (`es`), Swedish (`sv`), and French (`fr`) seeds are generated from English via `scripts/gen_app_i18n_de.py` / `gen_app_i18n_es.py` / `gen_app_i18n_sv.py` / `gen_app_i18n_fr.py` (venv + `deep-translator`). Settings → **Interface language** sets `uiLocale`. `reloadAppStrings` runs `applyUiI18n()` and `refreshSettingsUI()` so toggles pick up translations. Restart the app to apply the native menu bar language after changing locale |
