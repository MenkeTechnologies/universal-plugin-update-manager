...
| **Waveform Preview** | 800-subdivision min/max envelope waveform with gradient fill (cyan→magenta) and RMS center detail line. Seekable — click anywhere on the now-playing bar to jump; the expanded sample metadata row waveform seeks the same way when that file is the current track. 60fps playhead cursor. Right-click to toggle expand setting. **Playback is highest priority:** now-playing and expanded-row waveform/spectrogram work is **`requestIdleCallback`-scheduled** (with a long timeout fallback and `cancelIdleCallback` on track/row change) so fetch/decode/FFT never run on the hot path right after `play()`. Each new track bumps a generation counter so in-flight decode for a previous track is abandoned. Expanded-row waveform + spectrogram run sequentially and share one decode when the waveform was not already cached; the spectrogram FFT loop yields every few columns. File browser shows full-width waveform background behind each audio row with live playback cursor |
