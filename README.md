...
frontend/
  ...
  js/
    ...
    audio-engine.js    -- Audio Engine tab + `enginePlaybackStart` / `playback_*` for library output through the AudioEngine; mirrors floating-player parametric EQ (canvas + FFT), volume, speed, pan, and engine transport (play/pause, ±5s seek, reverse); tab layout matches **Settings** (`settings-container` / `settings-section` / `settings-row`)
