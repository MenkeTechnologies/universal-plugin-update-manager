...
Unit tests live in `src/**/*.rs` inside `#[cfg(test)]` modules. Integration tests live in `tests/*.rs` (one Cargo test binary per file), each importing `app_lib`. To tally `#[test]` functions without hardcoding: `cd src-tauri && rg -c '#\[test\]' src` and `rg -c '#\[test\]' tests`. **Total `cargo test` invocations** (sum of each binary’s `running N tests` line, including `0`): from `src-tauri/`, `cargo test 2>&1 | rg 'running [0-9]+ tests' | awk '{s+=$2} END {print s}'`. A few tests are `#[ignore]` (long migrations, heavy DB batches, optional stress); run `cargo test -- --ignored` to include them.

**`app_lib` unit tests — modules in `src/`**

| Module | Tests | Coverage |
|--------|-------|----------|
| **xref** | 59 | Ableton .als gzip XML (VST2/VST3/AU), REAPER .rpp plaintext (VST/VST3/AU/CLAP), Bitwig binary scan, FL Studio ASCII+UTF-16LE, Cubase Plugin Name markers, Logic AU name matching, Studio One ZIP+XML, DAWproject ZIP+XML, Pro Tools AAX paths+markers, Reason binary scan, all 5 plugin types (VST2/VST3/AU/CLAP/AAX), cross-format dedup, trailing junk handling, name normalization, `PluginRef` JSON round-trip, 3 real-file tests (FLP=7, CPR=2, LOGICX=13 plugins verified) |
| **history** | 42 | Scan CRUD, 50-scan limit, diff (added/removed/version-changed), pure `compute_plugin_diff` (duplicate path → last old row wins; downgrade emits `version_changed`; empty snapshots), pure `compute_*_diff` for empty audio/DAW/preset scans, KVR cache CRUD, audio history CRUD, DAW history CRUD, ID generation, preference storage (TOML + path-keyed pref cache for parallel tests) |
| **kvr** | 34 | Version parsing, version comparison, HTML version extraction (6 formats), download URL extraction, platform keyword detection, date filtering (incl. `2024.12.1`-style rejection), `1..2` parse segments, transitivity spot checks |
| **scanner** | 28 | Plugin type mapping, file size formatting, directory size calculation with depth limit, plugin discovery (non-recursive: only immediate children), VST directory enumeration, architecture detection, edge cases |
...
| **app_i18n** | tally `rg -c '#\[test\]' src/app_i18n.rs` | In-memory `load_merged` (English base + locale overlay, `""`/`en` skip overlay, unknown locale keeps English-only keys); `app_i18n_en.json` parses; **per-locale** spot checks (`seed_json_*_*_differs_from_en` for representative keys); **`seed_json_safe_catalog_keys_all_locales_differ_from_en`** applies the same “safe key” rule as `i18n-anchor-keys.test.js` to all embedded seeds; **seed parity**: all locale JSON files share the same key set as English; **no empty values** in any shipped locale (`en`/`de`/`es`/`sv`/`fr`/`nl`/`pt`); **`appFmt` placeholders**: `de`/`fr`/`nl`/`pt`/`sv` strings contain the same `{name}` / `{n}` / … tokens as English where present; **Spanish** (`es`) uses those same English token spellings in `menu.*`, `ui.palette.*`, `ui.sp_*`, `confirm.*`, and **`toast.*`** (e.g. `{name}` not `{nombre}`) so `toastFmt`/`appFmt` substitution matches `ipc.js`; **native menu bar**: every `t("menu.…")` key in `native_menu.rs` exists in the English seed; **system tray**: every `t("tray.…")` key used when building the tray menu in `lib.rs` exists in the English seed |
...
| **key_detect** | 22 | Goertzel algorithm (440Hz detection, near-zero for absent frequencies), chromagram (pure A, pure C, C major chord, multi-octave A, bins bounded [0,1]), `profile_correlation` Pearson (identity = 1, rotation alignment, constant chroma → 0), key profile matching (C major triad, A minor triad, perfect correlation, shifted profile), detect_key (WAV, silence, 96kHz, 8kHz, nonexistent, unsupported) |
...
| **`test/i18n-placeholders.test.js`** | tally `rg -c '^test\\(' test/i18n-placeholders.test.js` | Every `{…}` segment in each `i18n/app_i18n_*.json` value matches `ipc.js` `appFmt` token syntax (`{name}`, `{n}`, … — ASCII word chars only); no stray `{`/`}` after stripping valid `{token}` segments. |
...
| **`test/i18n-seed-parity.test.js`** | tally `rg -c '^test\\(' test/i18n-seed-parity.test.js` | `de`/`fr`/`nl`/`pt`/`sv` values use the same `{token}` multiset as English per key; `es` keeps English placeholders for `menu.*` / `ui.palette.*` / `ui.sp_*` / `confirm.*`, and matches English `{token}` multiset for `menu.*` + `tray.*` (native shell UI); English defines all `app_i18n.rs` native menu bar + tray keys (mirrors Rust `seed_json_*` tests on the JS CI path). |
...
**Total `node:test` count:** run `node --test test/*.test.js` from the repo root and read `ℹ tests N` (single process, authoritative). `pnpm run test:js` splits files into batches; **sum** each batch’s `ℹ tests` line to get the same `N` (the last batch’s line alone is **not** the total). The table above does not sum to `N` because some suites use `describe`/`it` or generated `test()` loops.
...
- **No Tauri IPC** — no invoke/events; no live `scanner.js` / `plugins.js` integration (`i18n-appfmt.test.js` duplicates `appFmt` only, not IPC).
