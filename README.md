...
| **Keyboard Navigation** | Arrow keys/j/k to navigate table rows and file browser (Ableton-style: right enters dir, left goes up). Customizable shortcuts (Settings → Keyboard Shortcuts) cover tabs (including F-key rows for later tabs), playback, scans, palette, and actions; the help overlay summarizes defaults plus a “more” block bound to the same catalog |
