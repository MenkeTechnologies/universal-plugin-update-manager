...
| **Cache Manager** | Settings → Data shows 8 individual cache clear buttons (BPM, Key, LUFS, Waveform, Spectrogram, Xref, Fingerprint, KVR), **Run analysis** / **Stop analysis** (BPM/Key/LUFS background batch; stop completes after the current batch, above Clear All Caches), plus Clear All Caches. **Clear All Databases** (same section) wipes scan-history tables only; strings `ui.btn.clear_all_databases`, `ui.tt.clear_all_databases`, `confirm.clear_all_scan_databases`, `toast.all_scan_databases_cleared`, and `menu.clear_all_databases` (Data menu + command palette). All caches stored in SQLite |
