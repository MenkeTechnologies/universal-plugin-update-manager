...
*Full PTY-backed terminal (xterm.js + portable-pty) with 256-color and truecolor support, drag-to-dock snapping to any corner, and persistent geometry. Toggle with Ctrl+T.*
...
| **ALS Generator** | Generates full Ableton Live Set files (**.als**, gzipped XML) from the scanned sample library — techno / schranz / trance arrangements across 27 track types (kick/clap/snare/hat/perc/ride/fill, bass/sub, lead/synth/pad/arp, riser/downlifter/crash/impact/hit/sweep-up/sweep-down, snare-roll/reverse/sub-drop/boom-kick, atmos/glitch/scatter, vox) over 7 sections (intro/build/breakdown/drop1/drop2/fadedown/outro). Key- and BPM-matched sample selection with per-type atonal toggles. Six group buses (**KICKS / DRUMS / BASS / BASS FX / MELODICS / FX**) emitted only when populated; group-level sidechain compression keyed to KICKS; return sends per category; master **Eq8** HPF (30 Hz) + **Limiter** (ceiling −0.3 dB) chain. Locators stamped at every section boundary labeled with the detected song key. **FL-Studio-style 6-lane × 8-bar-block timeline** (`frontend/js/als-timeline.js`) lets you paint / erase / ramp per-block overrides for **chaos / glitch / density / variation / parallelism / scatter**; **draggable section boundaries** (8-bar snap, min 8 / max 128 bars) persist per-genre under `alsSectionLengthsByGenre`. **Deterministic seed** — text input + 🎲 randomize button + 🔒 Lock toggle (`#alsSeed` / `#alsSeedLocked`): pins every random choice so same seed + same config = bit-identical arrangement. `generate_als_project` resolves the seed via `config.seed.unwrap_or_else(rand::random)` and echoes it in the result JSON so the wizard can display "Seed: 12345" for "regenerate with same seed". Seeds travel as JSON strings (avoids `Number.MAX_SAFE_INTEGER` precision loss) and are deserialized by `deserialize_seed` in `src-tauri/src/als_project.rs` which accepts either number or string form. Seed threading uses a **thread-local `StdRng`** in `src-tauri/src/techno_generator.rs` (`init_gen_rng` / `clear_gen_rng` / `with_gen_rng`) seeded once at the top of `generate()` and cleared on every exit path, so all 10+ randomness sites (swoosh rotation, scatter hits, fills, glitches, parallelism thinning, variation chunking, chaos gaps, density accents, glitch edits, sample-shuffle tie-breakers) feed from the same seeded stream. Project filenames still carry a wall-clock `YYYYMMDD_HHMMSS` suffix so back-to-back locked runs never overwrite each other. `restoreAlsPrefs` clears the stored seed on startup when Lock is off so a previous session's seed never silently freezes every new generation. Multi-song mode concatenates N songs with a 32-bar gap; each song re-rolls sample selection from the pool so arrangements diverge. IPC: **`generate_als_project`**, **`cancel_als_generation`**, **`als_query_samples`** (dry-run preview), plus blacklist / whitelist helpers. Frontend: `frontend/js/als-generator.js` (wizard) + `frontend/js/als-timeline.js` (canvas). See [`docs/tutorial/19-als-generator.html`](docs/tutorial/19-als-generator.html) for the full walkthrough |
...
| **Embedded Terminal** | Full PTY-backed terminal emulator (xterm.js DOM renderer + `portable-pty` Rust crate). Login shell (`zsh -l`) with `TERM=xterm-256color`. 256-color palette and truecolor (24-bit RGB) support via static CSS overrides and monkey-patched `_addStyle` for release WebKit compatibility. Drag-to-dock snapping to any corner (shared dock overlay with player, pixel-based zone positioning). Resizable from all 8 edges/corners via shared `modal-drag.js`. Geometry and dock position persisted to prefs. Toggle with **Ctrl+T**. Kill & close button tears down the PTY session. Respawns on keypress after session exit. `ResizeObserver` re-fits the terminal on pane resize. Backend: `src-tauri/src/terminal.rs` — `terminal_spawn`, `terminal_write`, `terminal_resize`, `terminal_kill` IPC commands. Frontend: `frontend/js/terminal.js` |
