...
| **Batch Selection** | Checkbox column in inventory tables (Samples, DAW, Presets, MIDI, PDF) for multi-item operations. **Selections are stored per tab** (`batchSetForTabId` in `batch-select.js`) — the floating bar count and actions apply only to the **active** table; switching tabs clears selection (`switchTab` → `deselectAll`). Select all/deselect, batch favorite, copy paths, export selected as JSON |
...
| **Visualizer Tab** | 6 real-time audio displays in 3×2 grid or single mode: FFT spectrum (log/linear), oscilloscope waveform (color picker), scrolling spectrogram (speed control), stereo field, peak/RMS level meters (hold indicator), 10-band octave analyzer. **Data:** tiles prefer **`playback_status.spectrum`** from the **`audio-engine`** sidecar when output is running (library playback, Audio Engine tone/preview, etc.); sample rate for bin mapping uses **`spectrum_sr_hz`**. **Fallback:** Web Audio **`AnalyserNode`** only when the sidecar is not supplying spectrum (local `<audio>` graph). Stereo/levels use spectrum-derived paths when on engine; true L/R Lissajous uses ChannelSplitter + dual analysers only in the Web Audio fallback. **UI:** chamfered HUD panels, neon toolbar chips, canvas HUD grid + rounded-top spectrum/band bars, phosphor-style stereo field, glowing waveform stroke. 30fps throttle, pre-allocated buffers, zero CPU when tab hidden. Fullscreen mode (Escape to exit). Trello drag tiles. Context menus for per-tile params. Tile canvases cache `CanvasRenderingContext2D` references; log-scale FFT draws at most 1024 columns; spectrogram rendering steps frequency bins (≈256 vertical bands max) to cut fill calls |
...
| **Parametric EQ** | Visual frequency response curve with draggable band nodes (Low/Mid/High). Real-time FFT spectrum overlay at 60fps via Web Audio **AnalyserNode** when the `<audio>` graph is active, or via **`playback_status.spectrum`** when library playback runs through the **audio-engine** sidecar. Log frequency axis (20Hz–20kHz), drag to change frequency and gain on the Web Audio nodes; **EQ gain** is pushed to the sidecar during drag (`playback_set_dsp`). Sidecar audio uses fixed band frequencies (see Audio Engine tab row). **Floating player:** showing the EQ section or expanding the player reapplies canvas height from prefs and restarts the draw loop after a double **`requestAnimationFrame`** (release WKWebView can leave the strip blank if only **`ResizeObserver`** is relied on). **`parametricEqTick`** wraps draw in **try/catch** so a one-off error does not stop the loop |
