...
| **Preset Scanner** | Indexes preset files (FXP, FXB, VSTPRESET, AUPRESET, Ableton ADG/ADV, Kontakt NKI, etc.) into SQLite. **Presets tab chrome matches DAW Projects:** `audio-progress-bar` under the toolbar while scanning, `audio-stats` row, then a stacked **disk-usage** bar and legend — segment widths are **percent of total bytes per format** (`bytesByType` from `db_preset_filter_stats`; incremental scan fallback uses per-format **counts** so the bar still reflects the mix) |
