...
| **Audio Engine tab** | **Sidecar** `audio-engine` (cpal): **engine_state** (ping + stream in one IPC), **list_input_devices**, **get_input_device_info** / **get_output_device_info**, **set_input_device** (validate id) / **set_output_device**, **start_output_stream** / **stop_output_stream** (optional **`start_playback`** after **`playback_load`** for library PCM via Symphonia + ring + EQ/gain/pan in the sidecar), **playback_*** status/seek/DSP, **start_input_stream** / **stop_input_stream** (optional **buffer_frames**), **set_output_tone** (F32 **440 Hz** test sine, low gain), structured **buffer_size** (`range`/`unknown`), **stream_buffer_frames** when fixed buffering is applied. **F6** opens the tab. **Rust:** `audio_engine_invoke` (`src-tauri/src/audio_engine.rs`). **Dev:** `beforeDevCommand` builds `audio-engine`. **Release:** `externalBin` + `scripts/prepare-audio-engine-sidecar.mjs`. **JS:** `frontend/js/audio-engine.js` — prefs `audioEngineOutputDeviceId`, `audioEngineInputDeviceId` (input caps selector), `audioEngineTestTone`, `audioEngineBufferFramesOutput` / `audioEngineBufferFramesInput` (legacy `audioEngineBufferFrames` migrated on load); `get_input_device_info` per selected input; **`input_peak`** on running input stream; the input capture status line is in the **engine status** card (always visible) rather than only inside the collapsible input-devices section; while the tab is active and input capture is running, the UI polls **`input_stream_status`** on a ~100ms interval so the peak line updates live (stops when another tab is selected, the window is hidden, or capture stops; overlapping slow invokes are skipped). Failed actions re-fetch **`engine_state`** to resync stream lines; missing **`audioEngineInvoke`** clears stream lines and shows **`ui.ae.err_no_ipc`** on the engine line. See `audio-engine/README.md`. |
...
**Library playback (`start_playback`):** **`playback_load`** probes the file with symphonia so **`src_rate`** matches the first decoded packet when **`codec_params`** from the container does not. **`start_output_stream`** with **`start_playback: true`** opens **[rodio](https://github.com/RustAudio/rodio)** on the selected device and prefers the same **F32** sample rate / stereo layout as before. **`PlaybackShared`** replaces the previous **`Player`** when a new load or stream restart runs so detached playback cannot continue after **Stop** or a new load. **`buffer_frames`** is **capped at 8192** in the sidecar (and in **`audio-engine.js`** prefs) so mistaken values like `144000` do not create multi‑second hardware buffers that keep playing after **Stop stream**. See `audio-engine/README.md`.
...
frontend/
  ...
  js/
    ...
    audio-engine.js    -- Audio Engine tab + `enginePlaybackStart` / `playback_*` for library output through the sidecar
