...
| **SQLite Backend** | All data stored in a single `audio_haxor.db` SQLite database (WAL mode, 64MB cache). Persists audio samples, plugins, DAW projects, presets, PDFs, MIDI files (FTS5-backed search), per-domain scan history, KVR cache, waveform/spectrogram/xref/fingerprint caches, and app i18n. Designed for multi-million-row libraries without browser OOM: the UI loads samples, DAW projects, presets, PDFs, and MIDI from the DB in pages (bounded JS memory during scans). Per-cache clear buttons in Settings (BPM, Key, LUFS, Waveform, Spectrogram, Xref, Fingerprint, KVR) |
...
| **Incremental unified scan** | Stores each visited directory’s mtime in SQLite (`directory_scan_state`); on the next unified scan, subtrees whose directory mtime is unchanged are skipped (faster on large trees and network shares). Preference `incrementalDirectoryScan` (`on`/`off`, default `on` in `config.default.toml`). Details and limitations: [`incremental-scan.md`](incremental-scan.md) |
...
frontend/
  ...
  js/
    app.js             -- Startup, auto-load last scan, restore preferences; `applyInventoryCounts` keeps header strip + stats-bar totals identical per category (always `get_active_scan_inventory_counts`; standalone scans stream-insert into SQLite per batch so counts track committed rows; `applyInventoryCountsPartial` throttles DB refresh on progress)
