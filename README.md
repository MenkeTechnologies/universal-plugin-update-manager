...
audio-engine/
  src/main.rs          -- Sidecar binary: cpal output device discovery; JSON IPC (one line stdin → one line stdout). Commands: `ping`, `list_output_devices`, `set_output_device`, `plugin_chain` (stub)
...
frontend/
  ...
  js/
    ...
    audio-engine.js    -- Audio Engine tab: `audio_engine_invoke` IPC, device list + prefs
