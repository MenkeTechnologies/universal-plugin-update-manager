...
| **Toast & UI i18n** | Slide-in toasts plus HTML placeholders/titles/options (`data-i18n*`), confirm dialogs, help overlay, and native menu/tray labels load from SQLite `app_i18n` (`key`, `locale`, `value`). English defaults are seeded from `src-tauri/app_i18n_en.json` (generate via `scripts/gen_app_i18n_en.py`; toast keys are edited in `scripts/gen_toast_i18n_en.py`). German (`de`), Spanish (`es`), and Swedish (`sv`) defaults are seeded from `src-tauri/app_i18n_de.json`, `app_i18n_es.json`, and `app_i18n_sv.json` (regenerate with `scripts/gen_app_i18n_de.py` / `gen_app_i18n_es.py` / `gen_app_i18n_sv.py` using a venv + `deep-translator`). Settings → Appearance → **Interface language** sets `uiLocale` (`en`, `de`, `es`, or `sv`). The UI calls `get_app_strings` on load; `toastFmt` / `appFmt` resolve `toast.*`, `menu.*`, `ui.*`, etc. `applyUiI18n()` applies DOM strings after load. Missing keys fall back to English. Restart the app to apply the native menu bar language after changing locale |
