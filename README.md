...
| **Waveform Preview** | 800-subdivision min/max envelope waveform with gradient fill (cyan→magenta) and RMS center detail line. Seekable — click anywhere on the now-playing bar to jump; the expanded sample metadata row waveform seeks the same way when that file is the current track. 60fps playhead cursor. Right-click to toggle expand setting. Now-playing waveform is scheduled after playback starts so audio begins before fetch/decode; each new track bumps a generation counter so in-flight decode for a previous track is abandoned (rapid MP3 skipping no longer stacks full-file `decodeAudioData` work). Expanded-row waveform + spectrogram run sequentially and share one decode when the waveform was not already cached; the spectrogram FFT loop yields to the event loop every few columns so long files do not freeze the shell. File browser shows full-width waveform background behind each audio row with live playback cursor |
