...
| **Floating Player** | Draggable audio player that docks to any corner with quadrant zone UI. Resizable from all 8 edges/corners. Play/pause, loop, shuffle (default **S**, customizable in Settings → Shortcuts), seek bar, volume, speed (0.25x-2x), optional **reverse** playback (decodes the file and plays the buffer backwards through the same EQ chain), recently played (50 tracks), song search with fzf matching, favorite/tag buttons. Expanded mode adds 3-band EQ, preamp gain, stereo pan, mono toggle, A-B loop. 60fps waveform playhead via requestAnimationFrame. The header mini-spectrum FFT reuses one pass over analyser bins for the filled curve and stroke (no per-frame `getElementById`). **Spectrum strip height** is adjustable via the horizontal drag handle under the FFT (24–200px, persisted as `npFftHeight` in prefs). Player state/size/dock persisted across sessions |
| **Waveform Preview** | 800-subdivision min/max envelope waveform with gradient fill (cyan→magenta) and RMS center detail line. Seekable — click anywhere on the now-playing bar to jump; the expanded sample metadata row waveform seeks the same way when that file is the current track. 60fps playhead cursor. Right-click to toggle expand setting. **Playback is highest priority:** now-playing and expanded-row waveform/spectrogram work is deferred with **`setTimeout(0)`** (not `requestIdleCallback`, which is often starved in Tauri/WKWebView so those draws never ran); pending timers are cleared with `clearTimeout` on track/row change so fetch/decode/FFT never run on the synchronous path right after `play()`. **`resolveWaveformBoxSize`** waits for rAF until the waveform container has non-zero dimensions (otherwise bar count was 0 and the canvas stayed blank). **`ipc.js`** sets **`window.convertFileSrc`** for asset URLs used in decode. **Heavy decode** (peak extraction, expanded-row spectrogram FFT, reverse-playback PCM copy) runs in a **Web Worker** (`frontend/js/audio-decode-worker.js`). On first run the worker script must compile; **`preloadAudioDecodeWorker`** is scheduled from **idle** and **~650ms after splash fade** so the first WAV play is less likely to block the UI on that cold start. If the worker cannot fetch `convertFileSrc` URLs (e.g. some WebView builds), the main thread **fetches** the file and **transfers** the `ArrayBuffer` to the worker so `decodeAudioData` still runs off the UI thread; expanded-row visuals no longer fall back to main-thread decode (which froze on multi‑GB WAVs). **Without** `Worker`, now-playing peaks and reverse mode still use main-thread `decodeAudioData` as a last resort. Reverse buffer reversal yields to the event loop periodically; after the worker returns decoded PCM, **staging into the playback `AudioBuffer` uses chunked `Float32Array.set` with yields between chunks** (a single `copyToChannel` was freezing the UI on multi‑GB WAVs). Each new track bumps a generation counter so in-flight decode for a previous track is abandoned. Expanded-row waveform + spectrogram share one worker decode when neither is cached. File browser shows full-width waveform background behind each audio row with live playback cursor; **list-row** mini waveforms (IntersectionObserver) use the same worker peak path (`decodePeaksViaWorker` / `window._decodePeaksViaWorker`) when available |
...
```
Cargo.toml (repo root) -- Cargo workspace: members `src-tauri` (`audio-haxor` app) and `audio-engine` (sidecar). Shared `Cargo.lock` and `target/` at repo root. Locale JSON must use the **same `{placeholder}` names** as English for `appFmt`; `scripts/sync_i18n_placeholders_from_en.py` copies English strings for any key that drifts (run after bulk locale edits if `cargo test` seed tests fail).

audio-engine/
  README.md            -- Sidecar protocol + build notes
  src/main.rs        -- cpal output stream (silence / F32 test tone), `engine_state`, `list_input_devices`, JSON line IPC
...
src-tauri/
  ...
  tauri.conf.json      -- Tauri app configuration + CSP + bundling + `externalBin` for `binaries/audio-engine`
...
frontend/
  ...
  js/
    ...
    audio-engine.js    -- Audio Engine tab: `audio_engine_invoke`, device list, cpal input/output stream start/stop
