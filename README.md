...
| **SQLite Backend** | All data stored in a single `audio_haxor.db` SQLite database (WAL mode, 64MB cache). Persists audio samples, plugins, DAW projects, presets, PDFs, MIDI files (FTS5-backed search), per-domain scan history, KVR cache, waveform/spectrogram/xref/fingerprint caches, and app i18n. Designed for multi-million-row libraries without browser OOM: the UI loads samples, DAW projects, presets, PDFs, and MIDI from the DB in pages (bounded JS memory during scans). Sample search uses a standalone `COUNT(*)` plus `LIMIT` (no `COUNT(*) OVER()`), so the engine does not materialize the full match before paging; the samples toolbar’s per-format counts use a debounced second query so filtering stays responsive on very large libraries. While any paginated `db_query_*` runs (plugins, samples, DAW, presets, PDFs, MIDI), the list or table shows a spinner and `requestAnimationFrame` yields so the WebView can paint; overlapping requests are dropped by sequence so slower queries cannot overwrite a newer filter. Sample queries (`db_query_audio`, `db_audio_filter_stats`) run SQLite on Tokio’s blocking pool; after each round-trip the samples tab yields (`setTimeout(0)`) before rebuilding table DOM so the filter field stays interactive during large `innerHTML` work. Per-cache clear buttons in Settings (BPM, Key, LUFS, Waveform, Spectrogram, Xref, Fingerprint, KVR) |
