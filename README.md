...
| **Audio Engine tab** | **Sidecar** `audio-engine` (cpal): **engine_state** (ping + stream in one IPC), **list_input_devices**, **get_input_device_info** / **get_output_device_info**, **start_output_stream** (optional **buffer_frames**) / **stop_output_stream**, **set_output_tone** (F32 **440 Hz** test sine, low gain), structured **buffer_size** (`range`/`unknown`), **stream_buffer_frames** when fixed buffering is applied. **F6** opens the tab. **Rust:** `audio_engine_invoke` (`src-tauri/src/audio_engine.rs`). **Dev:** `beforeDevCommand` builds `audio-engine`. **Release:** `externalBin` + `scripts/prepare-audio-engine-sidecar.mjs`. **JS:** `frontend/js/audio-engine.js` — prefs `audioEngineOutputDeviceId`, `audioEngineInputDeviceId` (input caps selector), `audioEngineTestTone`, `audioEngineBufferFrames`; `get_input_device_info` per selected input. See `audio-engine/README.md`. |
...
frontend/
  ...
  js/
    ...
    audio-engine.js    -- Audio Engine tab: `audio_engine_invoke`, device list, cpal stream start/stop
