...
| **`test/i18n-anchor-keys.test.js`** | tally `node --test test/i18n-anchor-keys.test.js` → `ℹ tests` | Auto-discovers **safe** `menu.*` / `tray.*` keys (all six non-`en` values differ from English — excludes e.g. `menu.app` / `tray.tooltip` where `AUDIO_HAXOR` is shared); one `test()` per key × locale plus a sanity case on safe-key count. |
