...
| **Cache Manager** | Settings → Data shows 8 individual cache clear buttons (BPM, Key, LUFS, Waveform, Spectrogram, Xref, Fingerprint, KVR) plus Clear All Caches. **Clear All Databases** (same section) wipes scan-history tables only; strings `ui.btn.clear_all_databases`, `ui.tt.clear_all_databases`, `confirm.clear_all_scan_databases`, `toast.all_scan_databases_cleared`, and `menu.clear_all_databases` (Data menu + command palette). All caches stored in SQLite |
...
Rust integration tests: `backend_handwritten_contracts.rs` (PDF snapshot/diff, `AudioQueryParams` JSON, KVR semver, similarity, xref `normalize_plugin_name`, BPM/LUFS/key/MIDI failure paths), `backend_handwritten_history_snapshots.rs` (`history` `build_*_snapshot` aggregates, `compute_*_diff` path semantics, `compute_plugin_diff` version rules, `ExportPayload` / snapshot serde, `daw_scanner` `ext_matches` / `is_package_ext`), and `backend_handwritten_parsers_kvr.rs` (KVR `extract_*` HTML, `KvrResult` / `UpdateResult` serde, `radix_string` / `gen_id`, xref `PluginRef`, fingerprint distance, `format_size` corners), and `backend_handwritten_db_ipc_json.rs` (serde JSON field names for `db` row/query types, `AudioMetadata`, `MidiInfo`, KVR cache DTOs, `FileWatcherState` defaults), and `backend_handwritten_history_serde_roundtrips.rs` (`history` diff/summary DTO JSON roundtrips, `AudioSample` optional-field serde keys, `pdf_meta` empty batch + missing file, `get_vst_directories` / `discover_plugins`, `similarity` fingerprint edge cases, `format_size` in `scanner` / `audio_scanner` / `daw_scanner` matching the crate root helper, `read_wav_pcm_pub` on empty file), `i18n_catalog_disk_and_ipc_guard.rs` (on-disk `app_i18n_*.json` filenames match `include_str!(../../i18n/…)` in `src/app_i18n.rs`, per-locale key parity with English, lexicographically sorted keys, ipc.js-style `{token}` placeholders, no UTF-8 BOM), and `app_i18n_test.rs` (SQLite `get_app_strings` merge, placeholders, `toast`/`confirm`/`help`/`ui.palette` non-empty across locales). `test/*.test.js` includes i18n catalog guards, static source checks, and VM-loaded `frontend/js` suites that exercise real implementations: `utils.js` (fzf-style `fzfMatch` / `scoreToken` / `searchScore` with deterministic score checks, `createETA`, `buildKvrUrl`, `findByPath`, `buildDirsTable`, `btnLoading`, regex highlight), `utils-fzf-comprehensive.test.js` (AND/OR aggregation, negation, regex vs fuzzy, gap scoring, `highlightMatch`, `slugify`, `buildDirsTable`), `filters-registry-fzf.test.js` (every `registerFilter` tab calls `searchScore` or `searchMatch`), `command-palette-source.test.js` (lazy plugin rows, `PALETTE_MAX` truncation), `ipc-source.test.js` (supported locales + null for unknown), `pure-data-helpers.test.js` (`findDuplicates`, `kvrCacheKey`, `XREF_FORMATS`), `xref-formats-sync.test.js` (pure-data list matches `xref.js`), `i18n-ui-source.test.js` (`applyUiI18n` empty keys / non-object guard), `multi-filter-source.test.js` (`getMultiFilterValues`), `export-source.test.js` (`resolveExportTitle`, formats), `dep-graph-source.test.js` (`buildDepGraphData` empty / orphan xref), `shortcuts-source.test.js` (`formatKey`), `columns-source.test.js` (`loadColumnWidths` / `saveColumnWidths`), `disk-usage-source.test.js` (`renderDiskUsageBar`), `settings-search-source.test.js` (settings modal row filter), `walker-status-source.test.js` (`_renderTile` idle vs scanning), `smart-playlists.js` (`matchesSmartRule` / `evaluateSmartPlaylist` with stubs and edge cases for BPM, tags, size, duration, unicode), `sort-persist-source.test.js` / `trim-common-indent.test.js`, `midi-note-name.test.js`, and extended `parseFzfQuery` / `parseToken` cases. Rust `pdf_meta` tests validate `lopdf` page counts using temporary PDFs generated with `printpdf`.
