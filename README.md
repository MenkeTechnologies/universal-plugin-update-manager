...
> **// SYSTEM ONLINE -- AUDIO_HAXOR v1.18.25 // by MenkeTechnologies**
...
| **PDFs tab (metadata)** | Background **`lopdf`** indexing fills the **Pages** column (`pdf_metadata` in SQLite). **Settings → Scan Directories → Background PDF metadata (auto)** (`pdfMetadataAutoExtract`, default **on** in `config.default.toml`) persists whether background extraction runs while the PDF tab is open (including on launch if that tab is restored). When **off**, only **Command Palette → Extract PDF metadata** runs a full pass; cached DB values still show in the table. Extraction runs only while the **PDF** tab is active **and** **`isUiIdleHeavyCpu()`** is false (same idle signal as the visualizer / audio paths). **`pdf_metadata_extract_abort`** cooperatively stops the Rust batch between **100-file** chunks; leaving the tab or going idle dispatches abort (`switchTab`, **`ui-idle-heavy-cpu`** in `pdf.js`). The user can stop at any time via the PDF toolbar **Stop metadata** button, Command Palette **Stop PDF metadata**, or **Stop All** (no “stopped” toast for the latter). **`pdf_metadata_get`** refreshes from progress events are **debounced (~300ms, trailing)** and **single-flight** to limit SQLite load. Unfinished work resumes via **`pdf_metadata_unindexed`** (paths without rows yet) |
