...
| **`test/i18n-value-safety.test.js`** | tally `rg -c '^test\\(' test/i18n-value-safety.test.js` | No C0 control characters, DEL, or U+2028/U+2029 in any catalog value; `de`/`es`/`fr`/`nl`/`pt`/`sv` `menu.scan_all` is not identical to English (translation spot-check, mirrors Rust `seed_json_*_menu_scan_all_differs_from_en`). |
