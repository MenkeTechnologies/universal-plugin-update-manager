...
| **Waveform Preview** | 800-subdivision min/max envelope waveform with gradient fill (cyan→magenta) and RMS center detail line. Seekable — click anywhere to jump. 60fps playhead cursor. Right-click to toggle expand setting. Now-playing waveform is scheduled after playback starts so audio begins before fetch/decode. File browser shows full-width waveform background behind each audio row with live playback cursor |
