...
**Rust cache (CI):** [Swatinem/rust-cache](https://github.com/Swatinem/rust-cache) must use `workspaces: .` (repo root). The virtual `[workspace]` writes **`target/`** at the repository root (`cargo metadata` → `target_directory`); `workspaces: src-tauri` would cache **`src-tauri/target`**, which this layout does not use and can misalign CI artifacts (e.g. Windows test runs). On **windows-latest**, the workflow runs **`cargo test --no-run`**, then removes **`*.dll`** (and stray `audio-engine.exe` / `.pdb`) **recursively** under `target/debug` so the harness does not load JUCE/VC runtimes from the wrong directory (`STATUS_ENTRYPOINT_NOT_FOUND`). A second **`cargo test`** would re-run **`tauri-build`** and copy those DLLs back, so CI runs each **`target/debug/deps/*-<rustc-hash>.exe`** harness with **`--quiet`** instead of invoking Cargo again. Enumeration is case-insensitive for **`.exe`**; **`build_script_build-*`** is skipped; **`$LASTEXITCODE`** is checked only when it is non-null (PowerShell treats **`$null -ne 0`** as true, which would otherwise exit 1 after successful runs); the step ends with **`exit 0`** so a stale exit code cannot fail the job.
