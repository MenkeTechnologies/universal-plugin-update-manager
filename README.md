...
| **Audio Scanner** | Discovers audio samples (WAV, FLAC, AIFF, MP3, OGG, etc.) with metadata extraction including duration, channels, sample rate, bit depth from file headers. Symlink deduplication via canonicalize with string-based fallback. Double-click any sample row to start playback, or enable **Single-Click Sample Playback** in Settings → Playback for one-click play. **Play on Keyboard Selection** (same section) autoplays when you move the Samples list selection with arrow keys or j/k. Floating music player with volume, playback speed, seek bar, and loop controls persists across all tabs |
