...
```bash
# Run all tests (JS + Rust `app_lib` unit tests + `behavioral_ultra` integration binary)
pnpm test
...
GitHub Actions (`.github/workflows/ci.yml`) runs `pnpm run test:js`, `cargo test --lib` and `cargo test --test behavioral_ultra` in `src-tauri/`, then `pnpm run tauri:build:ci` (`tauri build --ci --no-sign`) for unsigned release bundles on each OS. `db::init_global` is idempotent (safe when many `#[test]` threads race on the process-global SQLite handle), which avoids flaky `Database not initialized` failures on multi-core CI runners.
...
| **db** | 25 | SQLite insert/query roundtrip, fzf subsequence search, format filter, pagination (offset/limit), sort ascending/descending, BPM/key/LUFS update+retrieval, unanalyzed path query, aggregate stats, scan delete cascade, plugin/DAW/preset scan roundtrips, KVR cache roundtrip, clear_all_caches, per-table clear (bpm/key/waveform/xref/unknown), read/write cache waveform+xref, table_counts verification, concurrent `init_global` smoke |
