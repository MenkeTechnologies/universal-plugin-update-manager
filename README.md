...
> **// SYSTEM ONLINE -- AUDIO_HAXOR v1.18.17 // by MenkeTechnologies**
...
| **App Info** | Architecture and feature reference: build details (version, Tauri version, **rustc target triple** forwarded from `src-tauri/build.rs` via `cargo:rustc-env`, profile), supported file types by category (audio, plugin, DAW projects, presets, **MIDI**, **PDF**, plugin-extraction DAW types), analysis engines (BPM, Key, LUFS, Fingerprint), visualizers (6 types), export formats (5 types), storage backend, UI framework, search engine |
