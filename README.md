...
> **// SYSTEM ONLINE -- AUDIO_HAXOR v1.18.15 // by MenkeTechnologies**
...
| **Background Analysis** | Sequential BPM/Key/LUFS/metadata analysis starts as samples arrive during scan. Auto-pauses on user interaction, 100ms yield between batches, 50 files per **`batch_analyze`** IPC. Results are written to **`audio_samples`** (`bpm`, `key_name`, `lufs`, `bpm_exhausted`); SQLite errors propagate to the UI instead of being ignored. Cached to SQLite across reboots. Header badge shows an analyzing state while a batch runs and a cumulative count after each batch; the now-playing meta line picks up BPM/Key/LUFS when the current track is analyzed. **`batch_analyze`** (Rust) uses a **dedicated rayon pool** capped at **4** threads so a full batch does not claim every CPU core during a large scan — the previous default pool could starve system-wide audio I/O |
| **UI idle (heavy rAF)** | `frontend/js/ui-idle.js` combines **Page Visibility** (`document.hidden`) with Tauri **`WebviewWindow`** state (`isFocused`, `isMinimized`, `isVisible`, plus **`onFocusChanged`** / **`onResized`** to refresh minimize/visibility). While idle, the shell pauses **rAF**-driven work: floating-player playhead + mini FFT, engine-spectrum rAF, parametric EQ canvases, and the **Visualizer** tab loop. Dispatches document **`ui-idle-heavy-cpu`** (`detail.idle`) so listeners can cancel or reschedule; playhead time is **resynced** when idle ends. **Background BPM/Key/LUFS analysis** (`startBackgroundAnalysis` in `audio.js`) is **not** gated — it keeps using IPC batches and timers |
...
The **`pnpm run test:js`** suite is limited to tests that **read or execute repository sources**: **vm-loaded** frontend modules via `test/frontend-vm-harness.js`, **i18n** / **HTML** / **JS** catalog guards (`readFileSync` on `i18n/` and `frontend/`), plus the **audio-engine** IPC file (invoked only from `pnpm run test:audio-engine` / CI, not from `run-js-tests.mjs`). Standalone `node:test` files that only exercised inline algorithms (no app imports) were removed.
...
**Rust cache (CI):** [Swatinem/rust-cache](https://github.com/Swatinem/rust-cache) must use `workspaces: .` (repo root). The virtual `[workspace]` writes **`target/`** at the repository root (`cargo metadata` → `target_directory`); `workspaces: src-tauri` would cache **`src-tauri/target`**, which this layout does not use and can misalign CI artifacts (e.g. Windows test runs). On **windows-latest**, the workflow runs **`cargo test --no-run --color never`** with **all streams redirected to a temp file** (`*>` — plain lines for the **`Executable … (target\debug\deps\….exe)`** regex; colored output still breaks the parser; **`2>&1` into PowerShell arrays** wraps stderr as **`ErrorRecord`** and is brittle), then **`$LASTEXITCODE` from `& cargo`** before any pipeline. It parses those lines so every **unit-test** and **integration-test** harness matches **`cargo test`** on other OSes (globbing **`deps/*.exe`** by a fixed hash length is brittle and skipped most crates), then removes **`*.dll`** (and stray `audio-engine.exe` / `.pdb`) **recursively** under `target/debug` so the harness does not load JUCE/VC runtimes from the wrong directory (`STATUS_ENTRYPOINT_NOT_FOUND`). A second **`cargo test`** would re-run **`tauri-build`** and copy those DLLs back, so CI runs each parsed **`*.exe`** with **`--quiet`** via the **call operator** **`&`** from the workspace (same env/cwd as **`cargo test`**; **`Start-Process`** was unreliable for libtest exit codes) instead of invoking Cargo again.
