...
| `plugin_chain` | Scanned plugin list + **`insert_paths`** / **`slots`**. First request starts a **background** VST3/AU directory scan; responses use **`phase`: `"scanning"`** until done, then **`phase`: `"juce"`** with **`plugins`**. While **`phase` is `"scanning"`**, JSON includes **`scan_done`**, **`scan_total`** (candidate file counts), **`scan_skipped`** (modules skipped because the cached listing is up to date), **`scan_cache_loaded`** (whether `known-plugin-list.xml` was read), **`scan_current_format`** (`VST3` / `AU`), **`scan_current_name`** (plug-in currently being scanned). For **which bundle/path is being scanned** (and a monotonic **`scan_seq`** aligned with **`scan_done` + 1** for the next module), see **`engine.log`**: lines **`plugin scan: START …`** include **`file="…"`** (AU identifier or `.vst3` path), **`name="…"`**, **`format_pos`** within the format, and **`cache_listing_up_to_date`**. **`plugin scan: FAIL_SKIP …`** means **`scanNextFile`** threw: the identifier is **blacklisted** in-memory, appended to **`plugin-scan-skip.txt`** (same app-data folder, one full **`file=`** string per line), and the scan **continues** (progress stays in sync with JUCE’s scanner index). You can **pre-populate** that file to skip a known-bad AU/VST without loading it. The engine also **always excludes** Apple’s **AUVectorPanner** AU identifier (`AudioUnit:Panners/aupn,vbas,appl`) from the scan list — that module can **hang indefinitely** inside JUCE (no throw, so **`FAIL_SKIP`** never runs). Set **`AUDIO_HAXOR_PLUGIN_SCAN_SKIP`** to a comma-separated list of **additional** full identifiers to skip without editing the file. **`plugin scan: SKIP_LIST …`** in **`engine.log`** is logged when an identifier is removed by the merged skip list (file + built-ins + env). **`plugin scan: worker starting`** and **`plugin scan: VST3 phase complete; starting AU`** bracket phases on macOS. **`KnownPluginList`** is persisted to **`known-plugin-list.xml`** in the same app-data folder as the dead-man’s-pedal file so restarts reuse unchanged entries (see **Plugin scan cache files** below — the file exists only after a **successful** full scan). JUCE **dead-man's-pedal** file **`plugin-scan-dead-mans-pedal.txt`** lives in **`appDataDirectoryForSidecar()`**: the **parent directory of `engine.log`** when the host sets **`AUDIO_HAXOR_ENGINE_LOG`** (same as Tauri app data); if neither log env var is set (standalone sidecar), **`userApplicationDataDirectory/MenkeTechnologies/audio-haxor`**. It defers crashy modules to the end on later runs. **Native crashes** inside a vendor binary cannot be caught in-process — the **last `plugin scan: START` line** before a crash identifies the module; copy that **`file=`** value into **`plugin-scan-skip.txt`** to skip it on the next run. **Playback:** while **file output is running** (`outputRunning` + `playbackMode`), the scan worker **sleeps 2ms** after each module attempt so heavy **`scanNextFile`** work is less likely to starve the JUCE audio callback in the same process. |

## Plugin scan cache files

- **`known-plugin-list.xml`** — Same directory as **`plugin-scan-skip.txt`**, **`plugin-scan-dead-mans-pedal.txt`**, and (when the host sets it) the parent of **`engine.log`**: **`appDataDirectoryForSidecar()`**. With the Tauri app on macOS that is typically **`~/Library/Application Support/com.menketechnologies.audio-haxor/`**. If you run **`audio-engine` standalone** without **`AUDIO_HAXOR_ENGINE_LOG`** / **`AUDIO_HAXOR_APP_LOG`**, the directory is **`~/Library/Application Support/MenkeTechnologies/audio-haxor/`** instead — a **different** folder, so looking only under the bundle id path will not show files from a CLI run. The XML is **written only after the worker finishes both phases successfully** (VST3 and, on macOS, AU). If the scan **hangs**, the process **crashes**, or the worker ends in **`phase: failed`**, **`known-plugin-list.xml` is not created or updated** — which is why you may see **`engine.log`** and skip files but no cache until a scan completes end-to-end.
