...
**Process sits with no stdout when piping (even `ping`):** Older builds called **`AudioDeviceManager::initialise`** for output + input inside **`Engine`** construction, **before** the stdin thread ran. That can **block inside CoreAudio** in a bare pipe/Terminal context — so the binary appeared frozen and it was **not** a mutex deadlock. Current builds **defer** that initialisation until the **first non-`ping`** command so **`printf '%s\n' '{"cmd":"ping"}' | …/audio-engine`** can print a line without touching HAL first.
