...
`frontend/js/audio-engine.js` drives the Audio Engine tab and coordinates **`playback_*`** with the floating player. Library playback uses a **`playback_status`** WebView poll (~**250 ms**) when the window is focused and visible; when **`isUiIdleHeavyCpu`** (**`ui-idle.js`** — includes **`document.hidden`**, **`document.hasFocus()`**, Tauri window focus/minimize/visible), the WebView timer is **paused** and the Tauri **`audio_engine_eof_watchdog`** (~**1 Hz**) is the sole **`playback_status`** path so EOF auto-advance still works without doubling IPC when the app is in the background. **Open plug-in editor** compares the current UI insert paths to **`plugin_chain` `insert_paths`**; if they already match the loaded chain, it opens the native editor **without** stopping the output stream. If the UI differs (or the chain was never applied), it runs **`stop_output_stream`** then **`playback_set_inserts`** (no “inserts applied” toast — same as before) so **`playback_set_inserts`** never races an active stream. Behavior matches the root **`README.md`** Audio Engine / dev-vs-build sections (IPC guards, `engine_state` resync, input peak polling, etc.). On first panel refresh, if prefs request a different JUCE driver than the live process, **`set_audio_device_type`** is applied **only when no output/input stream is running** — otherwise the restore is deferred (that IPC **stops** streams, which would cut library playback). The **panel refresh** completes after device lists and caps; **VST3/AU directory scan** (`plugin_chain` → `phase: scanning`) continues in the **background** so the tab does not stay on “loading” for the full scan duration. While scanning, the UI **polls** `plugin_chain` and refreshes the Audio Engine plug-in progress line plus a **single** slide-in toast (updated in place, not stacked) — **`scan_done` / current plug-in name stay flat while JUCE blocks inside one slow module**, so the client shows **elapsed seconds** on that step. After **`AE_PLUGIN_SCAN_STUCK_HINT_SEC`** seconds on the same step (see `frontend/js/audio-engine.js`) it also appends **`ui.ae.plugins_scan_stuck_hint`**. **Hang vs crash:** the worker process stays alive while a **child** is stuck in `scanNextFile`; the parent eventually **kills** that child after **`AUDIO_HAXOR_PLUGIN_SCAN_TIMEOUT_SEC`** (see **`TIMEOUT_KILL`** / **`TIMEOUT_SKIP`** in **`engine.log`**). If **`plugin_chain`** still returns JSON, the main AudioEngine did not exit. The **dead-man’s-pedal** file still helps **crashy** plug-ins on **later** runs. **Restart Audio Engine** is only needed if the parent itself wedged (rare) or you changed the binary. **Insert chain** fuzzy pickers default to **effects only**: the WEB UI filters **`plugin_chain` `plugins`** by **`isInstrument`** (JUCE `PluginDescription::isInstrument`) unless the user enables **Show instruments** (preference **`aeInsertPickerShowInstruments`** = **`on`**). It also defaults to **hiding Audio Units** (preference **`aeInsertPickerHideAudioUnits`** = **`on`** when unset; flip to **`off`** to include them) because AU plugin editor windows render as a permanent 1×1 placeholder in this build — `audiocomponentd` refuses to deliver out-of-process AU view controllers (`_RemoteAUv2ViewFactory` XPC) to ad-hoc signed hosts. The *audio* path of an AU plugin still works fine (no UI required), so users who only need to drop a known-configured AU into the chain can disable the filter; users who expect to open the editor should stick to VST3. The limitation goes away the day the project is signed with a real Apple Developer ID — the helper `.app`, hardened runtime entitlements, `[NSApp finishLaunching]`, and `activateIgnoringOtherApps:YES` calls are all in place and only need a real `TeamIdentifier` to make `audiocomponentd` accept us. **`playback_set_inserts`** normalizes **`pluginFormatName`** for **`AudioUnit:`** identifiers (e.g. **`AU` → `AudioUnit`**) so JUCE’s format manager can match; if loading still fails with JUCE’s “no compatible plug-in format”, the engine replaces that with a clearer message when **`fileMightContainThisPluginType`** fails (e.g. **AudioComponentFindNext** — plug-in uninstalled or stale listing — use **Wipe cache & rescan**).
