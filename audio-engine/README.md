...
| `playback_set_loop` | `loop` (bool) | Full-file loop on the **forward** `AudioFormatReaderSource` path. **Reverse** decode has no reader loop (UI may still show loop for Web Audio / prefs). |
