 ...
 * - `seed_json_appfmt_placeholders_preserved_de_el_es_fi_fr_it_nl_pl_pt_ru_sv_zh_ja_ko_da_nb_tr_cs_hu_ro` (Rust; here: strict token multiset parity for `de`/`el`/`es`/`fi`/`fr`/`it`/`nl`/`pl`/`pt`/`ru`/`sv`/`zh`/`ja`/`ko`/`da`/`nb`/`tr`/`cs`/`hu`/`ro`; JS multiset is stricter than Rust substring check)
...
test('de, el, es, fi, fr, it, nl, pl, pt, ru, sv, zh, ja, ko, da, nb, tr, cs, hu, ro: appFmt placeholder token multiset matches English for every key', () => {
