...
- **Source of truth:** `i18n/app_i18n_en.json` (sorted keys). **CI** runs `pnpm run i18n:sort:check` (same logic as `scripts/sort_app_i18n_catalogs.py --check`) before the JS suite, and `test/i18n-catalog-files.test.js` asserts every shipped `app_i18n_*.json` has **lexicographically sorted** top-level keys — `merge_i18n_keys.py` / `sync_locale_keys_from_en.py` emit sorted maps; if you hand-edit JSON, re-sort before push (`pnpm run i18n:sort` → `scripts/sort_app_i18n_catalogs.py`) or CI fails with a list of files that need sorting. (`sync_i18n_placeholders_from_en.py` also rewrites sorted maps.)
...
**Updating English copy for an existing key:** `sync_locale_keys_from_en.py` does not overwrite values in other locales. To copy the new English string into every non-English catalog as a stub until translators re-run `gen_app_i18n_*.py`):

```bash
python3 scripts/sync_i18n_values_from_en.py ui.example.key
```
...
.venv-i18n/bin/python scripts/gen_app_i18n_id.py
.venv-i18n/bin/python scripts/gen_app_i18n_hi.py
.venv-i18n/bin/python scripts/gen_app_i18n_ro.py
.venv-i18n/bin/python scripts/gen_app_i18n_uk.py
