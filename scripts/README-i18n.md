...
## Other locales (`de`, `es`, `sv`, `fr`, `nl`, `pt`, `it`, `el`, `pl`, `ru`, `zh` — Simplified Chinese)
...
If English already contains new keys and you need the same keys in `de`/`es`/`sv`/`fr`/`nl`/`pt`/`it`/`el`/`pl`/`ru`/`zh` with English placeholder text until a full `gen_app_i18n_*` run:
...
- `test/i18n-anchor-keys.test.js` — for keys where **de/el/es/fr/it/nl/pl/pt/ru/sv/zh** all differ from English, none of those locales may copy `en` verbatim.
